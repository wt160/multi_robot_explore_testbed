...
#include "multi_robot_interfaces/srv/wfd_service.hpp"
...
class WfdServiceServerNode: public rclcpp::Node{
    public:
        using NavigateToPose = nav2_msgs::action::NavigateToPose;
        using GoalHandleNavigate = rclcpp_action::ClientGoalHandle<NavigateToPose>;
        using WfdService = multi_robot_interfaces::srv::WfdService;
        // using GoalHandleWfdService = rclcpp_action::ServerGoalHandle<WfdService>;

        WfdServiceServerNode(std::string robot_name, std::string mode);

        
        void handle_service_request(const std::shared_ptr<WfdService::Request> request,
        ...
        // rclcpp_action::GoalResponse handle_action_goal(const rclcpp_action::GoalUUID & uuid, std::shared_ptr<const WfdService::Goal> goal);
        // rclcpp_action::CancelResponse handle_action_cancel(const std::shared_ptr<GoalHandleWfdService> goal_handle);
        // void execute(const std::shared_ptr<GoalHandleWfdService> goal_handle);
        // void handle_action_accepted(const std::shared_ptr<GoalHandleWfdService> goal_handle);
    ...
    private:
        rclcpp::Publisher<nav_msgs::msg::OccupancyGrid>::SharedPtr window_frontier_publisher_;
        rclcpp::Publisher<nav_msgs::msg::OccupancyGrid>::SharedPtr local_frontier_publisher_;
        ...
        rclcpp::Service<WfdService>::SharedPtr service_server_;
